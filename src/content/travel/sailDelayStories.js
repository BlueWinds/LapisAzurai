...
`; },
  apply() {
    return g.map.delay = 3;
  }
};

window.stormLostCargo = function() {
  const cargo = choice(g.cargo);
  g.cargo = g.cargo.filter(c => c !== cargo);
  g.nextDayDescription = `Lost ${cargo.name} destined for ${places[cargo.to].name}`;
  return changeSection(1, true);
};

window.stormExtraDamage = function() {
...
window.stormExtraDelay = function() {
...
window.stormMakeProgress = function() {
  const distance = (Math.random() + 1) * Game.travelPxPerDay('Sail');
  g.map.speedBonus = distance;
  g.nextDayDescription = 'Kept sailing despite the danger';
  return changeSection(1, true);
};

window.stormReduceDamage = function() {
